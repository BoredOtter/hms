...
def get_medication(medication_name: str, db=Depends(get_db)):
    medication = db.query(Medication).filter(Medication.Medication_name == medication_name).first()
    if medication:
        return medication
    else:
...
def get_medication(medication_id: int, db=Depends(get_db)):
    medication = db.query(Medication).filter(Medication.ID_medication == medication_id).first()
    if medication:
        return medication
    else:
        raise HTTPException(status_code=404, detail="Medication not found")
...
def get_prescription(prescription_id: int, db=Depends(get_db)):
    prescription = db.query(Prescription).filter(Prescription.ID_prescription == prescription_id).first()
    if prescription:
        return prescription
    else:
